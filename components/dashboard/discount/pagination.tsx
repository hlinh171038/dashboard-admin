...
const Pagination:React.FC<PaginationParams> = ({
    ...
    return (
        <div
                className="flex items-center justify-end px-2 py-2 text-[15px] gap-2 "
                >
                    <button 
                        disabled = {Number(pageQuery) <=1}
                        onClick={()=>{
                            router.push(`/dashboards/discount/?search=${search}&page=${page -1}&per_page=${per_page}`)
                        }}
                    ...
                    <div className="flex items-center justify-between gap-2">
                        {
                            pagin && pagin.map((item)=>{
                                return (
                                    <div 
                                         onClick={()=>{
                                            router.push(`/dashboards/discount/?search=${search}&page=${item +1}&per_page=${per_page}`)
                                         }}
                                         key={item} 
