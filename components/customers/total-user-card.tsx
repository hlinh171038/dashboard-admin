    ...
    return (
        <div className="bg-slate-600 rounded-md p-2 relative">
            ...
            <div className="flex items-center justify-start gap-2">
                <div className="text-[35px] text-neutral-400 ">{users && users.length && users.length <10 ? `0${users.length}`: users.length}</div>
                <div className="text-[12px] ">
                    <div className="mb-[-2px] text-green-500">+ {totalUserThisWeek &&totalUserThisWeek.length < 10  ? `0${totalUserThisWeek.length}`: totalUserThisWeek.length }</div>
